...
DEFAULT_WEIGHTS = {
    "physical_loss_m3": 35,        # what a repair physically recovers
    "nrw_per_km_m3": 15,           # leak concentration - water per km of main
    "nrw_pct": 13,                 # efficiency signal, keeps rate in the frame
    "bursts_per_100km": 12,        # observed network deterioration
    "asset_age_index": 15,         # plant and meter condition
    "physical_share_pct": 10,      # share of loss that repair can address
}


def percentile_rank(s: pd.Series) -> pd.Series:
    """0-100 percentile rank. Chosen over min-max because plant size is heavily
    right-skewed: min-max would let the single largest plant compress all others
